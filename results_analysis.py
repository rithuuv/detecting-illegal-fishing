...
def make_predictions(X_train, X_cross, X_test, pickled_model):
    with open(pickled_model, 'rb') as f:  # Open the file in binary mode ('rb')
        model = pickle.load(f)
...
for gear in gears:
	...
	for classy in classifiers:
		...
		for model_num in models_by_columns:
			...
			pickled_model_file = '../results/' +model_num +'_'+ classy +'_'+ gear +'.pkl'
